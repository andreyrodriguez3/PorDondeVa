...
| ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A1**  | The spec never says where the admin dashboard is served from. Serving it on company/custom domains would place auth cookies on customer-controlled hostnames.                                                                           | §4, §24, §41  | Admin on a single fixed host (`admin.tubus.example`); company + custom hosts serve passengers only.                                                                                                                                                                                  | Resolved — shipped as built                                                                                                                                                                                                                                                                                                                    |
...
| **A6**  | §17 lists Schedule as part of a trip, but the DoD has the driver simply starting a trip. Are trips pre-generated from schedules?                                                                                                        | §14, §17, §43 | D16: `schedule_id` nullable, trips created on start, nearest departure auto-linked.                                                                                                                                                                                                  | Resolved — shipped as built                                                                                                                                                                                                                                                                                                                    |
| **A7**  | §24 defines `SUPER_ADMIN`, which by definition crosses the tenant boundary that §4 declares inviolable.                                                                                                                                 | §4 vs §24     | SUPER_ADMIN is a platform user (`company_id IS NULL`) restricted in the MVP to company lifecycle + domain management. It **cannot** read operational data (trips, locations, drivers) without an explicit, time-boxed, audit-logged impersonation grant — which is out of MVP scope. | Resolved — no `@Roles('SUPER_ADMIN')` endpoint touches tenant data anywhere in the codebase; the new `platform` module (A20) is its only surface                                                                                                                                                                                               |
...
| **A9**  | §16 says drivers should not have to pick arbitrary buses "if the company has configured an assignment", but no assignment model is specified.                                                                                           | §6, §16, §24  | MVP: `driver_profiles.default_bus_id`, pre-selected in the app, overridable at trip start (buses break down). A dated `bus_assignments` table is deferred.                                                                                                                           | Resolved — shipped as built                                                                                                                                                                                                                                                                                                                    |
...
| **A12** | §27 requires "no paid third-party infrastructure", but MapLibre needs a tile source, and OSM's public tile servers forbid production application use.                                                                                   | §27 vs §28    | `NEXT_PUBLIC_MAP_STYLE_URL` env var, defaulting to OpenFreeMap's hosted "liberty" style — a free, no-API-key, no-rate-limit tile source that's safe for production use (unlike OSM's own public tiles). Already what production uses; no separate decision needed.                   | Resolved — shipped as built                                                                                                                                                                                                                                                                                                                    |
...
| **A14** | §32 requires configurable retention but gives no default.                                                                                                                                                                               | §32           | `LOCATION_RETENTION_DAYS=90` for raw points; trips/live-state kept indefinitely. Nightly job deletes in bounded batches.                                                                                                                                                             | Resolved — shipped as built (`env.schema.ts`)                                                                                                                                                                                                                                                                                                  |
| **A15** | Nothing defines what happens when a driver forgets to end a trip. Trips would stay `ACTIVE` forever and appear on the passenger map.                                                                                                    | §17, §20      | Sweeper job: a trip with no new points for `TRIP_AUTO_END_MINUTES` (default 90) is auto-completed with `end_reason = AUTO_TIMEOUT`, and admins may end a trip manually.                                                                                                              | Resolved — shipped as built (`maintenance.service.ts::sweepStaleTrips`)                                                                                                                                                                                                                                                                        |
| **A16** | §24 says "assign" for buses and drivers without defining the semantics.                                                                                                                                                                 | §24           | Interpreted as: assign a default bus to a driver (A9), and assign a driver+bus to a trip at start. No standing shift-roster model in the MVP.                                                                                                                                        | Resolved — shipped as built                                                                                                                                                                                                                                                                                                                    |
...
| **A19** | **Gap.** §16 gives a driver `phone` but no email, yet the auth model (D15) makes email the login identifier. Bus drivers often have no work email.                                                                                      | §16, §25      | D20: nullable `email` (globally unique when present) + `username` (unique per company); driver app logs in with company code + username + password.                                                                                                                                  | Resolved — shipped as built                                                                                                                                                                                                                                                                                                                    |
| **A20** | **Gap.** DoD step 1 is "an administrator creates a company", but no surface existed for it — only the API and the seed script. Company creation is a `SUPER_ADMIN` action, and A7 keeps `SUPER_ADMIN` off the company-scoped dashboard. | §24, §43      | A small platform section at `admin.tubus.example/platform`, visible only to `SUPER_ADMIN`: create/suspend a company, set slug and timezone, create its first `COMPANY_ADMIN`, manage hostnames. Four screens, no separate app. Added to Phase 5.                                     | Resolved — shipped: `PlatformModule` (create/suspend/reactivate a company, create its first `COMPANY_ADMIN` with a one-time password) + `/admin/platform` UI. Custom-domain management still lives on each company's own `/settings` page (`COMPANY_ADMIN`-owned), not the platform section — different owner, not a gap                       |
| **A21** | **Gap.** No password recovery exists anywhere, and the MVP deliberately has no SMTP dependency. A driver who forgets a password would be unrecoverable.                                                                                 | §25           | Administrator-issued resets, no email needed: `COMPANY_ADMIN` resets operators and drivers, `SUPER_ADMIN` resets a company admin, and the one-time password is displayed once at issue and must be changed at next login. Self-service reset by email is deferred until SMTP exists. | Resolved — driver reset shipped (`PATCH /drivers/:id/password`, `COMPANY_ADMIN`-only) and `SUPER_ADMIN` creating a company's first admin (A20) issues a one-time password the same way. Operator self-reset (a `COMPANY_ADMIN` resetting another `COMPANY_ADMIN`/`OPERATOR`) is the one sub-case still open — small and additive, not blocking |
...
**Web:** `next`, `react`, `maplibre-gl`, `socket.io-client`, `motion`, `zod`, `tailwindcss`.
...
Rejected on the "minimal dependencies" principle: a client-side data-fetching/cache library (web state is `useState` + a manual `load()` re-fetch per page — small enough surface that React Query's cache invalidation machinery wasn't worth the dependency), a form library (same reasoning — plain controlled inputs), a component library, an i18n framework, an OpenAPI generator, a DI container for Android beyond Hilt, `class-validator` (Zod already covers validation and is shared with the web).
...
- **No client-side data-fetching library, on either surface.** Admin pages `useState` + re-fetch on demand; public pages are server components with no client data layer at all, so the passenger bundle stays small (§33). Revisit only if a page's manual re-fetch/cache-invalidation logic actually gets unwieldy — it hasn't yet.
...
Every question below has shipped as built and needs no further decision — the "working default already written into this document" that the earlier revision of this section promised is now the actual running system, not just a proposal. Kept here as a record of what was decided, not as an open list:

1. ~~**A1**~~ — Admin dashboard lives on a single fixed host (`admin.tubus.example`); company and custom domains serve passengers only.
2. ~~**A19 / A21**~~ — Driver login by company code + username (drivers have no email); administrator-issued password resets (driver reset by `COMPANY_ADMIN`, company-admin reset by `SUPER_ADMIN` via the platform section) in place of email recovery.
3. ~~**A20**~~ — `SUPER_ADMIN` platform section for creating companies: shipped (`PlatformModule` + `/admin/platform`).
4. ~~**A7**~~ — `SUPER_ADMIN` is restricted to company/domain lifecycle; no endpoint under that role touches tenant operational data.
5. ~~**A6 / A9 / A15 / A16 / A22**~~ — Trips created on driver start, with `SCHEDULED` unreachable in the MVP; one default bus per driver, overridable at trip start; automatic trip completion after 90 minutes of silence.
6. ~~**A23**~~ — The passenger route page offers a direction selector, since variants are directional (D6).
7. ~~**A14**~~ — 90-day raw-location retention (`LOCATION_RETENTION_DAYS`).
8. ~~**A12**~~ — Production map tile source: OpenFreeMap's hosted "liberty" style (free, no API key, production-safe), configurable via `NEXT_PUBLIC_MAP_STYLE_URL` if that ever needs to change.

The one genuinely open sub-item across all of these is a narrow slice of A21: a `COMPANY_ADMIN` resetting another `COMPANY_ADMIN`/`OPERATOR`'s password (as opposed to a driver's, or a company's first admin) has no endpoint yet. Small and additive, not blocking anything.
